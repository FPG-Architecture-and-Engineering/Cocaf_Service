    <div class="br-mainpanel">
        <div class="br-pageheader">
            <nav class="breadcrumb pd-0 mg-0 tx-12">
                ...
                <span class="breadcrumb-item active">Email Notification</span>
            </nav>
            <div class="control">
        ...
        <div class="br-pagetitle">
            <h4>Email Notification Records</h4>
        </div>
                    ...
                    <div class="modal-body pd-25">
                        <div class="row mg-b-10">
                            ...
                            <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                                <select name="app" class="form-control select" style="width: 100%" data-placeholder="Choose Application" required>
                                    <option value=""></option>
                                    <?php 
                                        foreach ($data['app'] as $key => $value) {
                                            echo '<option value="'.htmlDecode($value['app']).'">'.htmlDecode($value['app']).'</option>';
                                        }
                                        unset($key, $value);
                                    ?>
                                </select>
                        ...
                        <div class="row">
                            <label class="col-sm-3 form-control-label">User: <span class="tx-danger">*</span></label>
                            <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                                <select name="account_id[]" class="form-control select" style="width: 100%" data-placeholder="Choose User" multiple="multiple" required>
                                    <?php 
                                        foreach ($data['user'] as $key => $value) {
                                            echo '<option value="'.htmlDecode($value['id']).'">'.htmlDecode($value['first_name']).' '.htmlDecode($value['last_name']).'</option>';
                                        } 
                                    ?>
                                </select>
                            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.submit', function(e){
            ...
            if($('#form .required').length <= 0) {
                $.ajax({
                    url: '/system/emailNotification_json/',
                    type: 'post',
                    data: $('#form').serialize(),
                    beforeSend: function(){
            ...
            $(document).on('click', '.edit', function(e){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        $('select[name=app]').val(data.app);
                        $('select[name="account_id[]"]').val(data.account_id.split('-')).trigger('change');
